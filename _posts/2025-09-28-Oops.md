...
I've  managed to break my bparrot install on an old laptop and there are 3 files that I want to keep, so for now they'll live here
...
```bash
echo "--ID--";id;echo "--ENV--";env;echo "--SUID--";find / -perm -u=s -type f 2>/dev/null; echo "--CAPABILITIES--"; getcap -r / 2>/dev/null; echo "--EXPORTS--"; cat /etc/exports;echo "--CRONTAB--"; cat /etc/crontab;echo "--APP ARMOR ENTRIES--";ls /etc/apparmor.d/; echo "--- TIMERS ---- "; systemctl list-timers;echo "--SUDO--"; sudo -l;echo "--LISTENING SOCKETS--"; netstat -tulpn
```
...
Now that we've got a SOCKS proxy setup, (let's just say that it's on port 8080)
